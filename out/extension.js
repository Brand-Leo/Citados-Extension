...
function activate(context) {
    ...
    const generateCmd = vscode.commands.registerCommand('text-prompt-generator.generate', async () => {
        const text = core_1.TextSelector.getSelected();
        if (!text) {
        ...
        await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: 'Generando prompt...' }, async () => {
            try {
                const prompt = await core_1.PromptGenerator.generate(text, instruction);
                await core_1.PromptGenerator.show(prompt);
    ...
    const configCmd = vscode.commands.registerCommand('text-prompt-generator.config', async () => {
        ...
        if (key) {
            await core_1.Config.setApiKey(key);
